...
triangle, the homepage introduced itself with `SENDSET` in letterspaced caps, the
link-preview card said "One guide your client can actually use" (copy retired
before the object was named "Sendset"), and a published Sendset signed off in a
grey the client could barely see.

**Shipped, in order:**

| Surface | Result | Commit |
|---|---|---|
| Favicon + app icon | purpose-built simplified mark; 16/32/48 `.ico`, `icon.svg`, 180px apple icon | `c7f856c` |
| Marketing OG card | the homepage's own headline + the lockup, regenerated by the capture script | `1fc2414` |
| Homepage header | the horizontal lockup replaces the letterspaced word, 34px mobile / 48px desktop | `a926faf` `963b2c4` `105ed68` |
| Recipient signature | approved icon + **Made with Sendset**, linked, in `subtle`; print says the same words, text-only | `ba6f83f` `5d869cf` |

**Decisions that constrain future work:**

- **One copy of each asset**, in `public/brand/`. The marketing capture reads the
  lockup from there rather than keeping its own — a logo beside the capture
  script is a logo that can drift from the logo on the site.
- **The favicon is the simplified artwork**, not the detailed icon. The detailed
  mark's text lines and card edges merge into a smudge at 16px; this was checked
  by rasterising the live `.ico` at 16/32/48 and magnifying it, not by eye.
- **The recipient page gets a signature, not an advertisement.** Icon plus four
  words, `--sg-subtle` (8.2:1, where the old line was 40%-opacity grey), no
  button, no "create your own", no wordmark. It links to `sendset.io` in a new
  tab, matching the existing recipient external-link convention.
- **Print is text-only** — no icon (ink), no link (paper). The live URL beside it
  is the address a reader can act on.
- **Email carries no signature at all.** A sender's own message is not a place
  for our mark.
- **Guards, not conventions.** The header lockup must be ≤56px and the one
  permitted `.svg` on the homepage; the signature must use `subtle` and never
  `faint`; the printed tail must contain no anchor and no image. Each is
  mutation-checked. The 56px ceiling replaced a 40px one written around a
  smaller mark — it exists to stop a hero graphic, not to pin a size.

**Deliberately not done:**

1. **No lockup in the product.** `CreatorNav` stays plain links by its own
   design note; the login, `/join` and `/invited` screens still use a text
   `<h1>Sendset</h1>`. Adding a restrained lockup to those sparse screens was
   discussed and never approved.
2. **No web manifest.** The 192px and 512px PNGs from the brand set are not in
   the repo; they only earn their place alongside a manifest decision.
3. **No reverse/dark lockup in use.** The brand reference sheet named
   `sendset-logo-horizontal-reverse.svg`; no surface needs it yet.
4. **`og.png` and `og-recipient.png` are retained but referenced by nothing** —
   compatibility assets for previews already cached by messaging apps, which do
   not re-crawl on our schedule. Deleting either breaks previews in threads
   already sent to clients.
5. **Homepage logo sizing is open** for the broader homepage review. It took
   three passes (24/30 → 28/36 → 34/48) because two people independently read
   the smaller sizes as an eyebrow rather than a brand.
6. **The tagline never appears in product UI.** The brand sheet shows a
   `GATHER. ORGANIZE. SHARE.` lockup; it is not used anywhere and should not be.
