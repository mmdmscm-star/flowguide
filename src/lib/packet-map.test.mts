...
test("NO SCHEMA MIGRATION WAS INTRODUCED", () => {
  ...
  // deliberately added: 0050 (draft revisions + publication table) and 0051
  // (review-pending marker) belong to the published-snapshot track, not this slice.
  const migrations = readdirSync("supabase/migrations").filter((f) => f.endsWith(".sql")).sort();
  const highest = migrations[migrations.length - 1];
  assert.equal(highest, "0051_finalize_review_pending.sql",
