...
test("the Organize panel offers DONE, and no Cancel", async () => {
  const host = await mount();
  await click(byText(host, /^Organize$/)!);
...
test("organization SURVIVES Done — and Done writes nothing at all", async () => {
  ...
  assert.ok(byText(host, /^Organize$/), "the normal Library did not come back");
