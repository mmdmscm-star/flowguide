...
import { moveItem, moveSection, moveGroup } from "@/lib/library-service";
...
export async function POST(request: Request) {
  ...
  const { kind, id, direction } = body as { kind?: string; id?: string; direction?: string };
  ...
  const move = kind === "section" ? moveSection : kind === "group" ? moveGroup : kind === "item" ? moveItem : null;
  if (!move) {
