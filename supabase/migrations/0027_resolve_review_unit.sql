...
begin
  ...
  if v_unit is null then raise exception 'review: unit % not found on run %', p_unit_id, p_run_id; end if;
...
-- Service role only, exactly as every other ingestion lifecycle function.
revoke all on function public.resolve_review_unit(uuid, uuid, text, text)
