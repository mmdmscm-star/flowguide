...
--    subtext. The block is bound to p_packet_id: it is rejected unless it exists
--    AND belongs to that packet, so a caller cannot combine one packet's id with
--    another packet's block id. Rejects item blocks. Positions/item set untouched.
-- ----------------------------------------------------------------------------
...
--    positions. The block is bound to p_packet_id (rejected unless it belongs
--    there). Item blocks cannot be deleted here, and NO item content is touched:
--    only the single heading row is removed. Adjacent items keep their relative
--    order and simply close the gap.
-- ----------------------------------------------------------------------------
